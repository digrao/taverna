...
export function loadVaultBudgetConfig(_vaultPath: string): BudgetConfig {
  const configPath = join(dirname(new URL(import.meta.url).pathname), '..', '..', 'taverna.config.yaml')
  ...
  try {
    ...
    if (typeof budget['global_tokens_daily'] === 'number') cfg.tokens_daily = budget['global_tokens_daily']
...
export function checkBudget(vaultPath: string, project: string, config: BudgetConfig): BudgetCheck {
  ...
  const entries = project === '__global__' ? allToday : allToday.filter(e => e.project === project)
  ...
  if (config.tokens_daily !== undefined) {
    ...
    if (spent_tokens >= config.tokens_daily) {
      emitEvent({ event: 'budget_exceeded', project, tokens_spent: spent_tokens, tokens_limit: config.tokens_daily })
      return { allowed: false, spent_tokens, spent_usd, limit_tokens: config.tokens_daily, pct }
    }
    if (pct >= warnAt) {
      emitEvent({ event: 'budget_warning', project, pct_used: pct, tokens_spent: spent_tokens, tokens_limit: config.tokens_daily })
    }
  ...
  if (config.usd_daily !== undefined) {
    ...
    if (spent_usd >= config.usd_daily) {
      emitEvent({ event: 'budget_exceeded', project, tokens_spent: spent_tokens, usd_spent: spent_usd, usd_limit: config.usd_daily })
      return { allowed: false, spent_tokens, spent_usd, limit_usd: config.usd_daily, pct }
    }
    if (pct >= warnAt) {
      emitEvent({ event: 'budget_warning', project, pct_used: pct, tokens_spent: spent_tokens, usd_limit: config.usd_daily })
    }
...
export function getBudgetStatus(vaultPath: string, globalConfig: BudgetConfig): BudgetStatus {
  ...
  const projects: ProjectBudgetStatus[] = Array.from(byProject.entries()).map(([id, { tokens, usd }]) => ({
    id,
    spent_tokens: tokens,
    spent_usd: usd,
  }))
