...
export interface SchedulerOptions {
  dryRun?: boolean
  /** Max agent iterations per project (1 = single task, N = drain) */
  maxTasksPerProject?: number
  /** Max projects to dispatch */
  runBatchSize?: number
...
export async function runScheduler(
  ...
  const ctx: TavernaContext = { vaultPath: config.vaultPath, config, notificationBus }
  const now = new Date()
